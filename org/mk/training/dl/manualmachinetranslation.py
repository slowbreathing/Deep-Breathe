...
def seq2seq_model(input_data, target_en_data, dropout_prob, fr_len, en_len, max_en_len,
    ...
    lt_input=LookupTable.getInstance("input_word_embedding",fr_embeddings_matrix)
    ...
    encoding_op, encoding_st = encoding_layer(rnn_cell_size, fr_len, n_layers, encoding_embed_input,
                                              dropout_prob)
    print("encoding_op:",repr(encoding_op)," encoding_st:",encoding_st,type(encoding_st))
...
def make_model():
    ...
    if(encoder_type is "bi" and n_layers%2 == 0):
