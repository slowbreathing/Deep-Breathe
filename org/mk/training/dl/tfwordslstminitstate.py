...
"""
c = array_ops.ones([1, 5])
h = array_ops.ones([1, 5])
initstate = (c, h)
...
c = array_ops.ones([1, 5])
h = array_ops.ones([1, 5])
initstate = tf.nn.rnn_cell.LSTMStateTuple(c, h)

def RNN(x, weights, biases):
    with variable_scope.variable_scope(
            "other", initializer=init_ops.constant_initializer(0.1)) as vs:
        cell = rnn_cell.LayerNormBasicLSTMCell(n_hidden, layer_norm=False)
        outputs, states = tf.nn.dynamic_rnn(cell, x, dtype=tf.float32,initial_state=initstate)
